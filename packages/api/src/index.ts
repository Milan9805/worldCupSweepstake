...
function toV1Event(event: APIGatewayProxyEventV2) {
  ...
  return {
    ...
    resource: '',
    stageVariables: null,
    // Carry the client IP through so IP-based logic (e.g. admin-login rate
    // limiting) sees the real source instead of a constant 'unknown'. HTTP API
    // (v2) events expose it at requestContext.http.sourceIp.
    requestContext: {
      identity: { sourceIp: event.requestContext.http.sourceIp },
    } as unknown as APIGatewayEventRequestContextWithAuthorizer<undefined>,
    multiValueHeaders: {},
